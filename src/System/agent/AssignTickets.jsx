...
const AssignTickets = () => {
  ...
  const gettingAssignTickets = async () => {
    setLoading(true);
    const data = await getRequest("/assign-to-me", auth);
    if (data.ok) {
  ...
  useEffect(() => {
    if (auth && auth?.token) gettingAssignTickets();
  }, [auth && auth?.token]);
      ...
      <div className="table-responsive">
        <table className="table mt-1 agent-table">
          ...
          <tbody>
            ...
            {picks?.map((x, index) => (
              <PickedTicketItems x={x} index={index} from="assignTc" />
            ))}
          </tbody>
        </table>
      </div>
