...
import { PutRequest, getRequest } from "../../Actions/Requests";
...
const AssignModal = ({ open, setOpen, auth, ticketId, pickedBy }) => {
  ...
  const esclatingTicket = async () => {
    ...
    if (ok) {
      if (!reason || !agent) {
        return toast.error("Please write reason and choose agent to handover this ticket!");
      } else if (reason && agent) {
        setLoading(true);
        const data = await PutRequest("/assign-ticket", { reason, ticketId, newAgentId: agent }, auth);
        if (data.ok) {
      ...
      <div className="d-flex flex-column align-items-start gap-2">
        ...
        <div className="col-md-12">
          <div className="form-group py-2">
            ...
            <select className="form-control" name="agent" value={agent} onChange={(e) => setAgent(e.target.value)}>
